...
import { PENALTY_META, Infraction } from '@/lib/db';
import { SearchResult } from '@/lib/search';
import { VerdictResult } from '@/lib/verdict';

interface VerdictCardProps {
  verdictResult: VerdictResult & { error?: boolean; message?: string };
  matchedItem: Infraction;
  matchScore: number;
...
export default function VerdictCard({
  ...
  const confidencePercent = Math.min(99, Math.round(matchScore * 100));
  const maxScore = searchCandidates[0]?.scores.final || matchScore || 1.0;
      ...
      <div className="verdict-card">
        <div className={`verdict-head ${PENALTY_META[verdictResult.penalty]?.cls || 'pen-caution'}`}>
          ...
          <div className="verdict-conf">
            最相符條款信心 <strong>{confidencePercent}%</strong>
          </div>
        </div>

        <div className="verdict-body">
          <div className="verdict-title">{matchedItem.title}</div>
          <div className="verdict-cat">
            <span>類別: {matchedItem.cat}</span>
            <span>•</span>
            <span>原書: {matchedItem.page}</span>
          </div>
          ...
          >
            <strong>官方定義:</strong> {matchedItem.definition}
          </p>

          <div className="verdict-reason">
            <strong>判罰推導步驟:</strong>
            <ul>
              {verdictResult.reasons?.map((r, idx) => (
                <li key={idx}>{r}</li>
              ))}
            </ul>
          </div>
          ...
          {matchedItem.note && (
            ...
            >
              ...
              <span>條款備註: {matchedItem.note}</span>
            </div>
          )}
          ...
          <div>
            ...
            <ul className="verdict-examples">
              {matchedItem.examples?.map((e, idx) => (
                <li key={idx} style={{ fontSize: '0.9rem', opacity: 0.9 }}>
                  {e}
                </li>
              ))}
            </ul>
          ...
          {verdictResult.aiExplanation && (
            <div className="verdict-ai-box">
              ...
              <div className="verdict-ai-content">{verdictResult.aiExplanation}</div>
            </div>
          )}
          ...
          <div className="verdict-actions">
            <button
              ...
              onClick={() => onTraceback(matchedItem.source, matchedItem.page)}
            >
        ...
        <div style={{ marginTop: '32px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {searchCandidates.slice(1, 6).map((c, idx) => {
              const candPen = c.item.cheating
                ? 'dq'
                : (c.item.penalty[ragLevel as keyof typeof c.item.penalty] || c.item.penalty.L2)[0];
              return (
                <div
                  key={idx}
